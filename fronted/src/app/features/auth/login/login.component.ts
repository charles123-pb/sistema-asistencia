            ...
            @for (u of allUsers(); track u.id) {
              <button
                ...
                <div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-[13px] font-bold"
                     [style.background]="u.role === 'admin' ? 'var(--amber)' : 'var(--accent)'">
                  {{ initials(u.name) }}
                </div>
            ...
            <div class="flex items-center gap-2.5 mb-5">
              ...
              <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-[11px] font-bold"
                     [style.background]="selectedUser()?.role === 'admin' ? 'var(--amber)' : 'var(--accent)'">
                  {{ initials(selectedUser()?.name ?? '') }}
                </div>
                <div>
                  <div class="text-[13px] font-bold">{{ selectedUser()?.name }}</div>
                  <div class="text-[11px] text-[var(--text3)]">
                    {{ selectedUser()?.role === 'admin' ? 'Administrador' : 'Docente' }}
                  </div>
                </div>
              </div>
            </div>
...
export class LoginComponent implements OnInit {
  ...
  loadUsers(): void {
    this.api.get<AppUser[]>('/users').subscribe({
      next: (users) => {
        // Ordenar: admin primero, luego teachers por nombre
      ...
      error: (err) => {
        ...
        // Si falla, usar usuarios locales como fallback
        const d = this.data.data();
        this.allUsers.set([d.admin, ...d.teachers.filter(t => t.status === 'active')]);
      }
    });
  }
  ...
  attemptLogin(): void {
    ...
    if (!u) return;
    this.auth.login(u.id, this.pin()).subscribe({
      next: () => {
